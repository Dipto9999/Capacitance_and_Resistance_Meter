...
class StripChart :
    ...
    def run(self, data):
        ...
        if t > -1 :
            ...
            if (current_freq > self.ylim * 1.5) or (current_freq < self.ylim / 1.5) :
    ...
    def generate_subplots(self, data):
        """
        Generates subplots for frequency, capacitance, and resistance from a DataFrame.

        Parameters:
        - data (DataFrame): A pandas DataFrame with 'Datetime', 'Frequency (kHz)', 'Capacitance (nF)', and 'Resistance (kOhm)' columns.
        """
        # Set up the subplot grid
        fig, axs = plt.subplots(3, 1, figsize=(10, 15), sharex=True)

        # Frequency subplot
        axs[0].plot(data['Datetime'], data['Frequency (kHz)'], color='tab:red')
        axs[0].set_title('Frequency Time Series')
        axs[0].set_ylabel('Frequency (kHz)')
        axs[0].grid(True)

        # Capacitance subplot
        axs[1].plot(data['Datetime'], data['Capacitance (nF)'], color='tab:blue')
        axs[1].set_title('Capacitance Time Series')
        axs[1].set_ylabel('Capacitance (nF)')
        axs[1].grid(True)

        # Resistance subplot
        axs[2].plot(data['Datetime'], data['Resistance (kOhm)'], color='tab:green')
        axs[2].set_title('Resistance Time Series')
        axs[2].set_xlabel('Datetime')
        axs[2].set_ylabel('Resistance (kOhm)')
        axs[2].grid(True)

        fig.tight_layout()
        fig.savefig(
            f'''Figures/Meter_TimeSeries_{data['Datetime'].iloc[0].replace(' ', '_').replace('-', '_').replace(':', '_')}.jpg''',
            format = 'jpg', dpi = 800
        )
    ...
    def export_csv(self) :
        if not self.data_df.empty :
            ...
            csv_path = f'''LogBook/Meter_{csv_df['Datetime'].iloc[0].replace(' ', '_').replace('-', '_').replace(':', '_')}.csv'''
            csv_df.to_csv(csv_path, index = False)

            self.generate_subplots(csv_df)
...
class App :
    ...
    def save_fig(self) :
        if not self.frequency_chart.data_df.empty :
            jpg_path = f'''Figures/Meter_CapacitanceStream_{dt.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.jpg'''
